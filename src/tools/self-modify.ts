 ...
 */
export async function requestSelfModify(intent: string, notify?: NotifyFn, targetRepo?: string): Promise<SelfModifyResult> {
  try {
    console.log(`[self-modify] Generating code for: ${intent}${targetRepo ? ` (repo: ${targetRepo})` : ''}`);
    const plan = await generateAndReviewCode(intent, notify);
    plan.targetRepo = targetRepo;
...
export async function executeSelfModifyPlan(
  ...
  try {
    const repo = plan.targetRepo ?? 'jarvis';
    const branch = plan.prBranch!;

    await createBranch(repo, branch);

    for (const file of plan.files) {
      console.log(`[self-modify] Pushing ${file.path} to ${repo} branch ${branch}`);
      await upsertFile(repo, file.path, file.content, `feat: ${branch}`, branch);
    }

    if (plan.npmPackage) {
      await addNpmDependency(plan.npmPackage, branch, repo);
    }
...
async function addNpmDependency(packageName: string, branch?: string, repo = 'jarvis'): Promise<void> {
  const { getFileContent } = await import('../github/client.js');
  const raw = await getFileContent(repo, 'package.json');
  if (!raw) throw new Error(`package.json not found in ${repo} repo`);
