 ...
 * Authenticates via pre-injected cookies (TWITTER_AUTH_TOKEN, TWITTER_CT0, TWITTER_TWID).
 */
export async function readTwitterContent(url: string): Promise<BrowseResult> {
  ...
  try {
    ...
    // Inject pre-authenticated cookies to skip the login flow entirely
    await page.context().addCookies([
      { name: 'auth_token', value: process.env.TWITTER_AUTH_TOKEN ?? '', domain: 'x.com', path: '/' },
      { name: 'ct0', value: process.env.TWITTER_CT0 ?? '', domain: 'x.com', path: '/' },
      { name: 'twid', value: process.env.TWITTER_TWID ?? '', domain: 'x.com', path: '/' },
    ]);
